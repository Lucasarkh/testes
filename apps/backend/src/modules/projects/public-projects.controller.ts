...
export class PublicProjectsController {
  ...
  async resolveTenant(@Req() req: any, @Query('host') hostQuery?: string) {
    ...
    let projectId: string | null | undefined = req.projectId as
      | string
      | null
      | undefined;
    let resolvedProject = req.project as
      | { id: string; slug: string; name: string; tenantId: string }
      | undefined;
  ...
  private normalizeHost(rawHost?: string | null): string | null {
    ...
    return (
      rawHost.toLowerCase().trim().replace(/\.$/, '').split(':')[0] || null
    );
  }
  ...
  private async resolveFromHost(host: string) {
    const mainDomain = (process.env.MAIN_DOMAIN || 'lotio.com.br')
      .toLowerCase()
      .replace(/\.$/, '');
    const mainApex = mainDomain.startsWith('www.')
      ? mainDomain.slice(4)
      : mainDomain;
    ...
    const isSubdomainOfMain =
      !isMainDomain &&
      (host.endsWith(`.${mainApex}`) || host.endsWith('.lotio.com.br'));
