...
export class SchedulingService {
  ...
  async updateProjectConfig(
    projectId: string,
    updateDto: UpdateSchedulingConfigDto
  ) {
  ...
  async createScheduling(
    ...
    if (!projectId || projectId === '')
      throw new BadRequestException('ID do projeto é obrigatório.');

    const config = await this.getProjectConfig(projectId);

    if (!userId && !config.enabled) {
      throw new BadRequestException(
        'Agendamentos estão desabilitados para este empreendimento.'
      );
    }
    ...
    if (existingSchedulings >= config.maxSimultaneous) {
      throw new ConflictException(
        'Este horário já está totalmente preenchido.'
      );
    }
    ...
    if (!userId) {
      ...
      if (!availableDays.includes(dayOfWeek as any)) {
        throw new BadRequestException(
          'Agendamentos não disponíveis neste dia da semana para este empreendimento.'
        );
      }
      ...
      if (timeString < config.startTime || timeString > config.endTime) {
        throw new BadRequestException(
          `Agendamentos disponíveis apenas entre ${config.startTime} e ${config.endTime}.`
        );
      }
      ...
      if (config.lunchStart && config.lunchEnd) {
        if (timeString >= config.lunchStart && timeString < config.lunchEnd) {
          throw new BadRequestException(
            'Este horário coincide com o intervalo de almoço.'
          );
        }
      }
      ...
      if (config.breaks && Array.isArray(config.breaks)) {
        for (const b of config.breaks as any[]) {
          if (timeString >= b.start && timeString < b.end) {
            throw new BadRequestException(
              'Este horário está bloqueado por um intervalo programado.'
            );
          }
        }
      }
    ...
    if (!finalLeadId && !leadName) {
      throw new BadRequestException(
        'É necessário selecionar um lead existente ou preencher o nome para um novo lead.'
      );
    }
    ...
    if (!finalLeadId && leadName) {
      ...
      if (realtorCode) {
        const link = await this.prisma.realtorLink
          .findFirst({
            where: {
              projects: { some: { id: projectId } },
              OR: [{ id: realtorCode }, { code: realtorCode }]
            }
          })
          .catch(() => null);
    ...
    this.notifications
      .onNewScheduling(
        tenantId,
        projectId,
        scheduling.project.name,
        scheduling.id
      )
      .catch((e) =>
        this.logger.error('Notification onNewScheduling', e.message)
      );
    ...
    if (!userId) {
      this.notifications
        ...
        .catch((e) =>
          this.logger.error('Notification onPublicSchedulingCreated', e.message)
        );
    }
  ...
  async getBusySlots(projectSlug: string, dateStr: string) {
    ...
    return Object.keys(slotCounts).filter(
      (time) => slotCounts[time] >= config.maxSimultaneous
    );
  }
