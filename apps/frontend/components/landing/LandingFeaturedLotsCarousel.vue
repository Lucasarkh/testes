    ...
    <ClientOnly>
      <div class="v4-lots-carousel-bleed">
        <Swiper
          ...
          :watch-overflow="true"
        >
          <SwiperSlide v-for="lot in displayedLots" :key="`featured-${availableLotSlideKey(lot)}`">
            <NuxtLink
              :to="lotPageUrl(lot)"
              class="v4-lot-card v4-featured-lot-card"
            ...
            >
              <div class="v4-lot-card-header">
                ...
                <div class="v4-lot-status" :class="featuredLotsStatusClassFn(lot)">{{ featuredLotsStatusLabelFn(lot) }}</div>
              </div>
        ...
        <div v-else-if="loading" class="v4-lots-carousel-status">
          Carregando lotes em destaque...
        </div>

        <div v-else-if="error" class="v4-lots-carousel-status v4-lots-carousel-status--error">
          {{ error }}
        </div>
      </div>
      ...
      <template #fallback>
        <div class="v4-lots-carousel-bleed">
          <div v-if="displayedLots.length" class="v4-lots-carousel-fallback">
            <NuxtLink
...
import {
  ...
  featuredLotsStatusLabel as featuredLotsStatusLabelFn,
  featuredLotsStatusClass as featuredLotsStatusClassFn,
